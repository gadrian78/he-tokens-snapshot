...
from modules.cache_utils import price_cache, market_cache, is_cache_valid, save_cache, clear_caches
...
def get_market_info(api, market, symbol, retries=3, delay=2, trade_batch_limit=1000):
    ...
    if cached and is_cache_valid(cached):
        return tuple(cached["value"])

    # special case for SWAP.HIVE
    if symbol == "SWAP.HIVE":
        market_cache[symbol] = {"value": (1.0, 0.0), "timestamp": time.time()}
        save_cache()
        return 1.0, 0.0
    ...
    try:
        ...
        if metrics:
            ...
            if last_price > 0:
                ...
                market_cache[symbol] = {"value": (last_price, volume), "timestamp": time.time()}
                save_cache()
    ...
    for attempt in range(retries):
        try:
            ...
            if all_trades:
                ...
                market_cache[symbol] = {"value": (price_hive, vol24), "timestamp": time.time()}
                save_cache()
            ...
            if "does not exists" in str(e).lower():
                ...
                try:
                    ...
                    if price_from_orders > 0:
                        ...
                        market_cache[symbol] = {"value": (price_from_orders, 0.0), "timestamp": time.time()} # No volume data from order book
                        save_cache()
                ...
                try:
                    ...
                    if market_data and len(market_data) > 0:
                        ...
                        if price > 0:
                            ...
                            market_cache[symbol] = {"value": (price, 0.0), "timestamp": time.time()}
                            save_cache()
                            return price, 0.0
    ...
    try:
        ...
        if metrics:
            ...
            if last_price > 0:
                ...
                market_cache[symbol] = {"value": (last_price, 0.0), "timestamp": time.time()}
                save_cache()
    ...
    market_cache[symbol] = {"value": (0.0, 0.0), "timestamp": time.time()}
    save_cache()
    return 0.0, 0.0
...
def get_hive_price_usd():
    ...
    if "HIVE" in price_cache and is_cache_valid(price_cache["HIVE"]):
        return price_cache["HIVE"]["value"]

    try:
        resp = requests.get(COINGECKO_API, params={"ids":"hive","vs_currencies":"usd"})
        resp.raise_for_status()
        price = resp.json().get("hive", {}).get("usd", 0)
        price_cache["HIVE"] = {"value": price, "timestamp": time.time()}
        save_cache()
        return price
    except Exception as e:
...
def get_btc_price_usd():
    ...
    if "BTC" in price_cache and is_cache_valid(price_cache["BTC"]):
        return price_cache["BTC"]["value"]

    try:
        resp = requests.get(COINGECKO_API, params={"ids":"bitcoin","vs_currencies":"usd"})
        resp.raise_for_status()
        price = resp.json().get("bitcoin", {}).get("usd", 0)
        price_cache["BTC"] = {"value": price, "timestamp": time.time()}
        save_cache()
        return price
    except Exception as e:
