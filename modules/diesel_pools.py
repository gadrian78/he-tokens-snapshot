...
from modules.cache_utils import is_cache_valid, save_cache, diesel_cache

from modules.regular_tokens import fetch_from_hive_engine
...
def get_diesel_pool_info(api, token_pair, retries=5, delay=3):
    ...
    cache_key = token_pair.upper()

    # ✅ Try from cache first
    cached = diesel_cache.get(cache_key)
    if cached and is_cache_valid(cached):
        debug_log(f"💾 Using cached pool info for {token_pair}")
        return cached["value"]

    # Proceed with live fetching if cache is missing or expired
    try:
...
def get_diesel_pool_holdings(api, account, retries=5, delay=3):
    """Enhanced diesel pool holdings fetching with better error handling and caching"""
